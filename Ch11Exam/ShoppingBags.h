...
public:
     ...
     */
    vector<vector<Item>> ShoppingBagCombinations(vector<Item> currBag, vector<Item> remainingItems, std::vector<std::vector<Item>> allBags = std::vector<std::vector<Item>>());
...
private:
    ...
    std::string bag_to_string(std::vector<Item> bag, size_t index = 0) const;

    double priceOf(std::vector<Item> bag, size_t index = 0) const;
...
private:
    ...
    // Helper function
    void MoveItemHelper(int iterator, int limit, std::vector<Item> currBag, std::vector<Item> remainingItems, std::vector<std::vector<Item>>& allBags);
