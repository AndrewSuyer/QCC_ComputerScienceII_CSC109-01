...
public:
     ...
     * @return roster
     */
    const std::vector<Student> &GetRoster() const; // (5 points)
     ...
     */
    void PassCourse(const std::string& fileName);
     ...
     */
    void LoadFile(const std::string& fileName);  // Load file into rosters
     ...
     */
    friend std::ostream& operator<<(std::ostream &out, const  Course& s);
...
private:
    ...
    std::vector<Student> roster;    //collection of Student objects
};
